...
// For nodemailer (example using Gmail)
...
Payment Method: ${orderData.buyer.paymentMethod}`;
  ...
  .then(telegramResponse => {
    // Now, send a confirmation email using nodemailer
    ...
    const mailOptions = {
      from: EMAIL_USER,
      to: orderData.buyer.email, // Send email to the buyer
...
Guero's Shop`
    ...
    transporter.sendMail(mailOptions, (error, info) => {
      ...
      res.json({ success: true, telegramResponse, emailResponse: error ? error.toString() : info.response });
    });
  })
  .catch(error => {
